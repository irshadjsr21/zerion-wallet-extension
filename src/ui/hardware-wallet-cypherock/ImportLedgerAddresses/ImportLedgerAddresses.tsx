...
import type { EvmDerivationSchemeName } from '@cypherock/web-evm-connector';
import { getAddresses, getWallets } from '@cypherock/web-evm-connector';
...
function AddressSelectList({
  ...
  const { evmApp, managerApp } = ledger;
  const COUNT = 5;
  const derivationPathMap: Record<DerivationPathType, EvmDerivationSchemeName> =
    {
      ledger: 'ledger',
      bip44: 'metamask',
      ledgerLive: 'legacy',
  ...
  const { data, fetchNextPage, isFetchingNextPage } = useInfiniteQuery({
    queryKey: [
      ...
      evmApp,
      managerApp,
      derivationPathMap[pathType],
    ],
    queryKeyHashFn: (queryKey) =>
      queryKey.filter((x) => x !== evmApp && x !== managerApp).join(''),
    queryFn: async ({ pageParam = 0 }) => {
      const wallets = await getWallets(managerApp);
      if (wallets.length <= 0) {
        throw new Error('No wallets present on device');
      }

      return getAddresses(evmApp, {
        type: derivationPathMap[pathType],
        from: pageParam,
        count: COUNT,
    ...
    () =>
      new Map(
        data?.pages.flatMap((items) =>
          items.map((item) => [item.address, item])
        )
      ),
    [data]
  );
