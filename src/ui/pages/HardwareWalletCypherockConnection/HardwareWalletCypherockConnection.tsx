...
import { verifyCypherockAccountImport } from 'src/ui/hardware-wallet/types';
import { accountPublicRPCPort, walletPort } from 'src/ui/shared/channels';
import { setCurrentAddress } from 'src/ui/shared/requests/setCurrentAddress';
import { useRenderDelay } from 'src/ui/components/DelayedRender/DelayedRender';
import { invariant } from 'src/shared/invariant';
import { CypherockIframe } from 'src/ui/hardware-wallet-cypherock/CypherockIframe';
...
export function HardwareWalletCypherockConnectionStart({
  onImport,
}: {
    ...
    async function handler(event: MessageEvent) {
      ...
      if (isRpcRequest(event.data)) {
        ...
        if (method === 'ledger/connect') {
          ...
          verifyCypherockAccountImport(params);
          if (onImport) {
            onImport(params);
          } else {
            finalize(params);
...
export function HardwareWalletCypherockConnection() {
  return (
    <FrameLayout>
      <Routes>
        <Route path="/" element={<HardwareWalletCypherockConnectionStart />} />
